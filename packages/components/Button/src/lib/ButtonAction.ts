...
const buttonAction: Action<HTMLElement, ActionButtonArgs | undefined> = (
	node,
	actionArgs,
) => {
	...
	function clickEffect(ev: MouseEvent) {
		...
		el.style.cssText =
			`width:${circunsference}px;height:${circunsference}px;top:${pos.y}px;left:${pos.x}px; opacity: 0.2; will-change:scale,opacity;`;
		node.appendChild(el);
		const keyframe = [
			{ scale: "0", opacity: 0.2 },
			{ scale: "1.2", opacity: 0 },
